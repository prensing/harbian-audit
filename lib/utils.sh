...
# Check after deleting blank lines and comment lines
does_valid_line_exist_in_fileset() {
    local FILESET=$1
    local LINE=$2
    local FILE
    
    debug "Checking if $LINE is present in $FILESET"
    FNRET=1
    for FILE in $FILESET; do
        if $SUDO_CMD [ -r "$FILE" ] ; then
            debug "$SUDO_CMD sed '/^#/d' $FILE | sed '/^[[:space:]]*$/d' | grep -Fq --'$LINE'"
	    if $($SUDO_CMD sed '/^#/d' $FILE | sed '/^[[:space:]]*$/d' | grep -Fq -- "$LINE"); then
                FNRET=0
                break
            fi
        else
...
# Check the value of an SSHD parameter
# Once a user restriction is configured, need to specify a user to check params
check_sshd_parameter()
{
    local PATTERN="$1"
    
    debug "Checking for $PATTERN in SSHD configuration"

    debug "sshd -T -C user=root | grep -qiE -- '$PATTERN'"
    if $(sshd -T -C user=root | grep -qiE -- "$PATTERN"); then
        FNRET=0
    else
        FNRET=1
    fi
